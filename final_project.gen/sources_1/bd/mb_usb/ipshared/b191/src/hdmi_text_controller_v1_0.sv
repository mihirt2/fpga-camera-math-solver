...
(
    ...
    input logic [18:0] cam_wr_addr,
    input logic [15:0] cam_wr_data,
    input logic        cam_we
);
...
// Camera frame buffer - 640x480 x 8 bits
(* ram_style = "block" *) logic [7:0] cam_fb [0:307199];

// Camera mode control
logic cam_mode;
assign cam_mode = 1'b1; // CHANGE when going to vitas

// Camera read - direct 1:1 mapping, no scaling
logic [18:0] cam_rd_addr;
logic [7:0]  cam_pixel;

// 640 = 512 + 128, so y*640 = (y<<9) + (y<<7)
assign cam_rd_addr = ({9'b0, drawY} << 9) + ({9'b0, drawY} << 7) + {9'b0, drawX};
...
always_ff @(posedge cam_pclk) begin
    if (cam_we)
        cam_fb[cam_wr_addr] <= cam_wr_data[15:8];  // Y channel only
end

// Grayscale output
logic [3:0] cam_gray;
assign cam_gray = cam_pixel[7:4];
